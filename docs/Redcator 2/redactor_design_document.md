...
### AI Integration: Claude Code via MCP
**Decision:** Redactor uses Claude Code (Anthropic's CLI agent tool) as its AI layer. The app spawns `claude -p` as a subprocess to trigger AI workflows programmatically. Claude Code connects to the app via an MCP server (stdio between Claude Code and a Python MCP proxy, which forwards over HTTP to the app's internal server).
**Rationale:** Claude Code can be triggered programmatically from the app — the clinician clicks a button (e.g., "Generate Notes") and the app spawns Claude Code as a subprocess with the appropriate system prompt and MCP tools. This removes the UX friction of requiring clinicians to manually switch to a separate app and type commands. Claude Code supports structured JSON output (`--json-schema`), multi-turn conversations (`--resume`), real-time streaming, and the same MCP server infrastructure.
**Installation requirement:** Claude Code must be installed on the clinician's machine (`brew install --cask claude-code` — zero dependencies). The app checks for `claude` in PATH on launch and shows a setup guide if not found. A paid Anthropic plan (Pro, Max, Teams, or Enterprise) is required.
**Architecture:** The app embeds system prompts (workflow instructions for each AI task) and spawns Claude Code with `--append-system-prompt`, `--allowedTools` (restricted to MCP tools), and `--output-format json` for structured responses. MCP tools provide Claude Code with access to redacted session data, client profiles, and write endpoints for generated content. The MCP server and HTTP server are identical to the previous Cowork architecture — only the initiation mechanism changes.
**Privacy model unchanged:** All content Claude Code sees is redacted. Entity maps with real names live in `Private/` and are never exposed via MCP tools. Claude Code runs locally and connects to the Anthropic API for inference — redacted content only.
**Future direct API migration:** The data layer, document storage, and UI are AI-transport-agnostic. If a direct Anthropic API integration is preferred later, the system prompts and MCP tool definitions translate directly to API tool-use calls. The HTTP server and MCP proxy become unnecessary — the app calls the API directly with the same prompts and tool schemas.
