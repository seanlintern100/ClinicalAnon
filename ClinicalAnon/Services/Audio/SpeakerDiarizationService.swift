...
class SpeakerDiarizationService: ObservableObject {
    ...
    /// FluidAudio's SpeakerManager for consistent speaker IDs across chunks
    /// Uses embedding similarity to maintain speaker identity between audio chunks
    ...
    func initialize() async throws {
        ...
        do {
            ...
            // Initialize SpeakerManager for cross-chunk speaker ID consistency
    ...
    /// Creates fresh SpeakerManager so the next chunk establishes new speaker identities
    func resetSpeakerTracking() {
        speakerManager = SpeakerManager()
#if DEBUG
        print("SpeakerDiarizationService: Speaker tracking reset - new SpeakerManager created")
#endif
    }
    ...
    func diarize(audioURL: URL) async throws -> [SpeakerSegment] {
        ...
        do {
            ...
            // Use SpeakerManager to assign consistent speaker IDs across chunks
            let segments = result.segments.map { segment -> SpeakerSegment in
                var finalSpeakerId = segment.speakerId

                // Use SpeakerManager for cross-chunk consistency if available
                if let manager = speakerManager, !segment.embedding.isEmpty {
                    if let speaker = manager.assignSpeaker(
                        segment.embedding,
                        speechDuration: segment.durationSeconds,
                        confidence: segment.qualityScore
                    ) {
                        finalSpeakerId = speaker.id
#if DEBUG
                        print("  [DEBUG] SpeakerManager mapped \(segment.speakerId) -> \(speaker.id)")
#endif
                    }
                }

                // Convert FluidAudio's "S1", "S2" format to numeric if needed
                let speakerId = finalSpeakerId.replacingOccurrences(of: "S", with: "")
